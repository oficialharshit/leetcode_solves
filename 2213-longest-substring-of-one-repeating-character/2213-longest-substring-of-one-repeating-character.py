class Solution:
    def longestRepeating(self, s, queryCharacters, queryIndices):
        ...
        def merge(node):
            ...
            if prefix[left] == length[left] and right_char[left] == left_char[right]:
                prefix[node] += prefix[right]

            suffix[node] = suffix[right]
            if suffix[right] == length[right] and right_char[left] == left_char[right]:
        ...
        def build(node, start, end):
            if start == end:
                ...
                prefix[node] = suffix[node] = best[node] = length[node] = 1
                return
